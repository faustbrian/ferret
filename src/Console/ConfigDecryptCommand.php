...
final class ConfigDecryptCommand extends Command
{
     ...
     *
     * @param FerretManager $manager The Ferret manager instance for handling decryption
     *
     ...
     * @throws InvalidConfigurationException  If decryption fails or data is invalid
     *
    ...
    public function handle(FerretManager $manager): int
    {
        ...
        if ($resolvedKey === null) {
            $this->components->error('No decryption key provided. Use --key or --app-key option, or set ferret.encryption.use_app_key to true.');
     ...
     *
     * @param FerretManager $manager  The Ferret manager instance
     ...
     *
     * @throws ConfigurationNotFoundException If the file does not exist
     * @throws InvalidConfigurationException  If decryption fails
     *
     * @return int Command exit status (SUCCESS or FAILURE)
     */
     ...
     *
     * @throws DirectoryNotFoundException    If the directory does not exist
     * @throws InvalidConfigurationException If decryption fails for any file
     *
     * @return int Command exit status (SUCCESS or FAILURE)
     */
    ...
    private function resolveDecryptionKey(?string $key, bool $useAppKey): ?string
    {
        ...
        $shouldUseAppKey = $useAppKey || config('ferret.encryption.use_app_key', false);
