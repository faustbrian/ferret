...
/**
 * Exception thrown when a configuration value is an array but a boolean was expected.
 *
 * This exception indicates a type mismatch where the configuration expects
 * a boolean type but encountered an array during validation or retrieval.
 *
 * @author Brian Faust <brian@cline.sh>
 */
final class BooleanValueCannotBeArrayException extends InvalidBooleanValueException
{
    /**
     ...
     * key expected a boolean but received an array instead.
     *
    ...
    public static function forKey(string $moduleName, string $key): self
    {
        return new self(sprintf(
            'Configuration value for [%s.%s] must be a boolean, array given.',
            $moduleName,
            $key,
        ));
    }
