...
/**
 * Exception thrown when configuration value is null but float was expected.
 *
 * Occurs during typed configuration access when attempting to retrieve a float
 * value but the underlying configuration data is null or unset.
 *
 * @author Brian Faust <brian@cline.sh>
 */
final class FloatValueCannotBeNullException extends InvalidFloatValueException
{
    /**
    ...
    public static function forKey(string $moduleName, string $key): self
    {
        return new self(sprintf(
            'Configuration value for [%s.%s] must be a float, null given.',
            $moduleName,
            $key,
        ));
    }
